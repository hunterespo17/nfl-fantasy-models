"""
Step 17 -- Build the TE projection model for the UPCOMING season.

Same shape as the receiver build (scripts\\16_build_wr_model.py), with the
differences a tight end room forces:

  * It keeps the top THREE tight ends on each depth chart, not four. The
    receiver build keeps four because a WR4 is still a real if small job -- 41%
    of routes, 2.9 targets a game. A TE4 is not: measured over 2018-2025 he runs
    18.6% of routes and sees a 3.5% target share, and there were only ten TE5
    seasons in eight years worth measuring at all. Three is where the drop is,
    and past it the rows are blockers and special-teams bodies.
  * It passes play-by-play through to the model, same as the receiver build.
    Routes are not in any public table, so the board estimates them as snap
    share times team dropbacks, and the dropback count comes from the pbp file.
  * It prints the name-match warnings the model collects. The snap table joins
    on names, not IDs, and this position list is worse for collisions than any
    other -- "j smith" is Jonnu Smith and Jeff Smith, "t hill" is Taysom Hill,
    "d smith" walks into Dalton Schultz's era. Where a name is ambiguous the
    model refuses to guess and says so here.
...
ONE THING TO KNOW BEFORE READING THE BOARD. Replacement level here is TE12 --
twelve teams, one starting tight end, and the flex spot is split between backs
and receivers only. That is the league setting in src/config.py, not a modelling
choice made in this file, and it is left exactly as it was found. If you start a
second tight end in the flex more often than not, the true replacement level is
deeper than 12 and every value-over-replacement number on this board is
conservative in the same direction for everybody -- so the ORDER is unaffected.
"""
import os
import pathlib
import shutil
...
from src import (  # noqa: E402
    adp as adp_mod, availability, config, current_roster, data, media, ratings,
    report, team_features, te_blend,
)

POS = "TE"
KEEP_DEPTH = 3          # TE1..TE3 on each depth chart
...
def _refresh_site() -> None:
    ...
    """
    print(f"\nThis board on its own:  {config.OUTPUT_DIR / 'te_model.html'}")
    try:
...
def main() -> None:
    ...
    stale = []
    if "Opportunity" not in getattr(te_blend, "DEFAULT_WEIGHTS", {}):
        stale.append("te_blend")
    # The ceiling. A copy without it would still build a board -- just one that
    # lets a two-target blocking tight end project eight points a game.
    if not hasattr(te_blend, "CEIL_SLOPE"):
        stale.append("te_blend")
    # The two-tier name matcher. Without it the board silently swaps route
    # shares between players who share a first initial and surname, which on
    # THIS position list is not a hypothetical.
    if not hasattr(te_blend, "_short_key"):
        stale.append("te_blend")
    # The position's own availability line. Without it every tight end is priced
    # as a running back's body -- see BASE_FIT in src/availability.py.
    if "TE" not in getattr(availability, "BASE_FIT", {}):
        stale.append("availability")
    ...
    # back to targets alone -- and at a position where the difference between a
    # starter and a blocker IS the route count, that is a different, worse
    # model than the one this script claims to build. Still wrapped, because a
    # degraded board beats no board, but it says so loudly.
    snaps = None
    try:
    ...
    debug_path = config.OUTPUT_DIR / "current_map_debug_te.txt"
    ...
    if cmap.empty:
        ...
        result = te_blend.run(weekly, team_season, players, config.SCORING,
    ...
    # the same as a wrong answer, so print it.
    ...
    n_te_adp = len(adp_mod.for_pos(adp_df, POS)) if not adp_df.empty else 0
    if n_te_adp == 0:
        print(f"  [note] No {POS} rows in data/adp.csv — floor/ceiling shown, "
              "ADP, value and risk skipped. Add rows with pos=TE to turn those on.")
    ...
    if not cv:
        print("  [note] No ADP expectation curve — 'worth the pick?' in points is skipped. "
              "Needs TE rows in data/adp_history.csv whose names match.")
    elif cv.get("source") == "board":
        print("  [note] ADP curve fell back to THIS year's prices (no TE history in "
    ...
    if _cal.get("anchor") == "drafted players":
        ...
        if _floor:
            print(f"     {len(_floor)} at the 0.0 floor (blocking tight ends the scale "
                  "bottoms out on): " + ", ".join(_floor[:8])
                  + (" ..." if len(_floor) > 8 else ""))
    elif _cal:
        print(f"  [note] Points scale fell back to ALL {_cal.get('n_all')} {POS}s, "
              ...
              f"joined by name and it needs {te_blend.MIN_CAL_ROWS}. Early-round tight ends "
    ...
    # Fitted at 24 of 829 historical seasons, so a handful here is right.
    ...
    print(f"  Workload ceiling ({te_blend.CEIL_BASE} + {te_blend.CEIL_SLOPE} x expected "
          f"targets) binds on {n_cap} of {len(result['payload'])} tight ends.")
    ...
    meta = {
        ...
        "subline": f"TE index-blend · top {KEEP_DEPTH} on each depth chart · "
                   f"{_scoring_label()}",
        "note": f"{len(result['payload'])} tight ends. Teams and depth from current depth "
                ...
                "Replacement level is TE12: one starter, twelve teams, no flex share.",
    }
    (config.OUTPUT_DIR / "te_model.html").write_text(report.render(result, meta), encoding="utf-8")
    report.save_board(result, meta, config.OUTPUT_DIR / "boards" / "te.json")
    ...
    pd.DataFrame(rows).to_csv(config.OUTPUT_DIR / "te_projections.csv", index=False)
    ...
    _refresh_site()
    print("If any team or depth spot looks wrong, open current_map_debug_te.txt and send it to me.")
...
if __name__ == "__main__":
    try:
        ...
        _p = config.OUTPUT_DIR / "current_map_debug_te.txt"
